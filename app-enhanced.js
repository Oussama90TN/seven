...
function handleExpenseSave(e) {
    ...
    const expenseData = {
        ...
        products: document.getElementById('expenseProducts').value,
                ...
                <div class="bg-white rounded-lg shadow p-4 mb-3">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <div class="text-right">
                            <p class="font-bold text-rose-400">${exp.price} QAR</p>
                            <p class="text-xs text-gray-600">${exp.payment}</p>
                        </div>
                    </div>
                    <p class="text-sm text-gray-700 mb-2">${exp.products}</p>
...
function addDeduction() {
    ...
    newItem.className = 'deduction-item flex gap-2 mb-2';
    newItem.innerHTML = `
        <input type="text" class="deduction-label flex-1 px-4 py-2 border border-gray-300 rounded-lg" placeholder="Deduction reason">
        <input type="number" class="deduction-amount w-32 px-4 py-2 border border-gray-300 rounded-lg" step="0.01" value="0" onchange="calculateTotal()">
        <button type="button" onclick="removeDeduction(this)" class="text-red-500 hover:text-red-700">
...
function addAdvance() {
    ...
    const newItem = document.createElement('div');
    newItem.className = 'advance-item flex gap-2 mb-2';
    newItem.innerHTML = `
        <input type="text" class="advance-label flex-1 px-4 py-2 border border-gray-300 rounded-lg" placeholder="Advance reason">
        <input type="number" class="advance-amount w-32 px-4 py-2 border border-gray-300 rounded-lg" step="0.01" value="0" onchange="calculateTotal()">
        <button type="button" onclick="removeAdvance(this)" class="text-red-500 hover:text-red-700">
            <i class="fas fa-times"></i>
        </button>
    `;
    container.appendChild(newItem);
                    ...
                    <div class="border-t pt-3 space-y-2">
                        ${sal.deductions.map(d => `
                            <div class="flex justify-between text-sm">
                                <span class="text-red-600">- ${d.label}</span>
                                <span class="text-red-600">${d.amount} QAR</span>
                            </div>
                        `).join('')}
                        ${sal.advances.map(a => `
                            <div class="flex justify-between text-sm">
                                <span class="text-orange-600">- ${a.label}</span>
                                <span class="text-orange-600">${a.amount} QAR</span>
                            </div>
                        `).join('')}
                    </div>
                ` : ''}
